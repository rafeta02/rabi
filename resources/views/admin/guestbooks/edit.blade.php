...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.guestbook.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.guestbooks.update", [$guestbook->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.guestbook.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $guestbook->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('ucapan') ? 'is-invalid' : '' }}" name="ucapan" id="ucapan">{{ old('ucapan', $guestbook->ucapan) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('kehadiran') ? 'is-invalid' : '' }}" name="kehadiran" id="kehadiran">
                    ...
                    @foreach(App\Models\Guestbook::KEHADIRAN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('kehadiran', $guestbook->kehadiran) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('guest') ? 'is-invalid' : '' }}" name="guest_id" id="guest_id">
                    @foreach($guests as $id => $entry)
                        <option value="{{ $id }}" {{ (old('guest_id') ? old('guest_id') : $guestbook->guest->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('guest'))
                    <div class="invalid-feedback">
